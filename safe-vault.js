...
function validateVaultAttachment(file) {
  if (!file) {
    return "";
  }

  const isImage = file.type.startsWith("image/");
  const isAllowedFile = isImage || VAULT_ATTACHMENT_LIMIT.allowedTypes.includes(file.type);

  if (!isAllowedFile) {
    return "Attach an image or PDF file.";
  }

  if (file.size > VAULT_ATTACHMENT_LIMIT.maxSize) {
    return "Attachment must be under 8 MB.";
  }

  return "";
}

function updateVaultAttachmentStatus(input, statusEl) {
  if (!input || !statusEl) {
    return;
  }

  const file = input.files?.[0];
  const error = validateVaultAttachment(file);

  if (!file) {
    statusEl.textContent = "No attachment selected";
    statusEl.dataset.type = "info";
    return;
  }

  statusEl.textContent = error || `${file.name} (${formatVaultFileSize(file.size)})`;
  statusEl.dataset.type = error ? "warning" : "success";
}
...
function renderVaultItems(items) {
  ...
  items.forEach((item, index) => {
    ...
    deleteButton.dataset.attachmentPath = attachment?.storagePath || "";
    ...
    if (attachment?.downloadURL && attachment?.name) {
      const attachmentLink = document.createElement("a");

      attachmentLink.className = "safe-vault-attachment-link";
      attachmentLink.href = attachment.downloadURL;
      attachmentLink.target = "_blank";
      attachmentLink.rel = "noopener";
      attachmentLink.textContent = `Open attachment: ${attachment.name}`;
      card.appendChild(attachmentLink);
    } else if (attachment?.name) {
      const attachmentLabel = document.createElement("span");

      attachmentLabel.className = "safe-vault-attachment-link";
      attachmentLabel.textContent = `Attachment saved: ${attachment.name}`;
      card.appendChild(attachmentLabel);
    }
...
async function initSafeVault() {
  ...
  const attachmentInput = document.getElementById("vaultAttachment");
  const attachmentStatus = document.getElementById("vaultAttachmentStatus");
  ...
  evidenceForm.addEventListener("submit", async event => {
    ...
    const attachmentError = validateVaultAttachment(attachmentFile);

    if (attachmentError) {
      setVaultMessage(attachmentError, "warning");
      attachmentInput?.focus();
      updateVaultAttachmentStatus(attachmentInput, attachmentStatus);
      return;
    }
    ...
    submitButton.disabled = true;
    submitButton.textContent = attachmentFile ? "Uploading..." : "Saving...";

    try {
      const evidenceRef = firebase.doc(firebase.collection(db, "users", currentUser.uid, "vaultItems"));

      if (attachmentFile) {
        const safeFileName = attachmentFile.name.replace(/[^\w.\-]+/g, "_").slice(0, 120) || "attachment";
        const storagePath = `users/${currentUser.uid}/vaultAttachments/${evidenceRef.id}/${Date.now()}-${safeFileName}`;
        const attachmentRef = firebase.ref(storage, storagePath);

        await firebase.uploadBytes(attachmentRef, attachmentFile, {
        ...
        evidence.attachment = {
          ...
          storagePath,
          downloadURL: await firebase.getDownloadURL(attachmentRef)
        };

        submitButton.textContent = "Saving...";
      }

      await firebase.setDoc(evidenceRef, evidence);
      evidenceForm.reset();
      updateVaultAttachmentStatus(attachmentInput, attachmentStatus);
      setVaultMessage(attachmentFile ? "Evidence note and attachment saved to your private vault." : "Evidence note saved to your private vault.", "success");
    } catch (error) {
      setVaultMessage("Could not save this note. Check your Firebase rules and connection.", "error");
  ...
  document.getElementById("vaultList").addEventListener("click", async event => {
    ...
    try {
      const attachmentPath = deleteButton.dataset.attachmentPath;

      if (attachmentPath) {
        try {
          await firebase.deleteObject(firebase.ref(storage, attachmentPath));
        } catch (storageError) {
          console.warn("Vault attachment delete warning:", storageError);
        }
      }
  ...
  attachmentInput?.addEventListener("change", () => {
    ...
    const attachmentError = validateVaultAttachment(attachmentInput.files?.[0]);
