...
function normalizeAnalysisResult(result, originalText) {
  ...
  const status = ["Safe", "Warning", "Harmful"].includes(safeResult.status) ? safeResult.status : "Safe";
  ...
  return {
    ...
    score,
    status,
    tone: safeResult.tone || "Neutral",
    insight: safeResult.insight || "Supportive or neutral language reads as safe.",
    flags,
    confidence: safeResult.confidence || (status === "Harmful" ? "High" : status === "Warning" ? "Medium" : "Moderate"),
  ...
  return normalizeAnalysisResult(
    {
      ...localResult,
      source: "local",
      confidence: localResult.status === "Harmful" ? "High" : localResult.status === "Warning" ? "Medium" : "Moderate",
...
async function updateDemo(message) {
...
function initDemo() {
  ...
  const runDemo = async () => {
    ...
    demoButton.textContent = "Analyzing...";

    try {
      await updateDemo(demoInput.value);
    } finally {
      demoButton.disabled = false;
      demoButton.textContent = "Analyze Message";
    }
  };
  ...
  void updateDemo(demoInput.value || "You are so weird and nobody likes you");
}
