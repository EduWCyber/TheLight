...
function getHandAnalyzerApiUrl() {
  ...
  if (isLocalPreview) {
    return `${window.location.origin}/analyze-hand`;
  }

  return configuredUrl || `${window.location.origin}/analyze-hand`;
}

function warmRenderBackend() {
  const analyzerUrl = document.querySelector('meta[name="analyzer-api-url"]')?.getAttribute("content")?.trim() || "";
  const handAnalyzerUrl = document.querySelector('meta[name="hand-analyzer-api-url"]')?.getAttribute("content")?.trim() || "";
  const urls = [analyzerUrl, handAnalyzerUrl].filter(Boolean);

  if (urls.length === 0 || !("fetch" in window)) {
    return;
  }
  ...
  const warmUrl = url => {
    ...
    fetch(url, {
      ...
      // The first request may fail while Render wakes. Opening the connection is still useful.
    }).finally(() => {
      if (timeoutId) {
...
function initHandAnalyzerPanel() {
  ...
  const switchButton = document.getElementById("handCameraSwitchButton");
  ...
  if (!statusEl || !insightEl || !checkButton || !startButton || !switchButton || !stopButton || !video || !canvas || !labelEl || !confidenceEl) {
    return;
  }
  ...
  const sendPrediction = async landmarks => {
    ...
    try {
      ...
      const prediction = String(result.label || result.prediction || "");
      const isPunch = prediction.toLowerCase() === "punch";

      setPrediction(prediction, result.confidence);
      setStatus(
        isPunch ? "Punch detected" : "Hand detected",
        isPunch ? "harmful" : "safe",
        result.message || "The hand analyzer reviewed the current hand pose."
      );
    } catch (error) {
      setStatus("Backend issue", "warning", "Camera is running, but the hand analyzer did not return a prediction.");
    } finally {
  ...
  const updateSwitchButton = () => {
    switchButton.textContent = preferredFacingMode === "user"
      ? "Use Back Camera"
      : "Use Front Camera";
    video.classList.toggle("is-back-camera", preferredFacingMode === "environment");
    canvas.classList.toggle("is-back-camera", preferredFacingMode === "environment");
  };
  ...
  const startCamera = async () => {
    ...
    setStatus("Starting camera", "ready", `The browser is asking for ${preferredFacingMode === "user" ? "front" : "back"} camera permission.`);
    ...
    try {
      ...
      hands.onResults(results => {
        ...
        void sendPrediction(landmarks);
      });

      const analyzeFrame = async () => {
        if (!isCameraActive || !hands || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) {
          frameRequestId = window.requestAnimationFrame(analyzeFrame);
          return;
        }
        ...
        if (!isSendingFrame) {
          ...
          try {
            ...
            setStatus("Camera issue", "warning", "The camera is open, but the hand scanner could not read this frame.");
          } finally {
            isSendingFrame = false;
          }
        }
      ...
      setStatus("Camera live", "safe", `Using the ${preferredFacingMode === "user" ? "front" : "back"} camera. Show your hand clearly in frame.`);
    } catch (error) {
      await stopCamera();
  ...
  switchButton.addEventListener("click", async () => {
    ...
    if (isCameraActive) {
      setStatus("Switching camera", "ready", "Changing camera view...");
      await stopCamera();
      await startCamera();
    }
  });
